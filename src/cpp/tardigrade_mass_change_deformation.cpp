...
namespace tardigradeStressTools {
    ...
    namespace massChangeDeformation {
        ...
        massChangeDeformationBase<num_params>::massChangeDeformationBase(
            const floatType &dt, const secondOrderTensor &At, const floatType &ct, const floatType &ctp1,
            const floatType &rhot, const floatType &rhotp1, const floatType &gammat,
            const std::array<floatType, num_params> &parameters, const floatType alpha, const floatType tolr,
            const floatType tola, const unsigned int maxiter, const floatType lsalpha, const unsigned int maxlsiter)
            : _dt(dt),
              _At(At),
              _ct(ct),
              _ctp1(ctp1),
             ...
             * \param &gammat: The previous rate multiplier for the evolution of the mass-change deformation (units:
             * None) \param &parameters: The parameter vector \param &alpha: The integration parameter (0 for explicit 1
             * for implicit). Defaults to the second-order accurate value of 0.5 \param tolr: The relative tolerance
        ...
        void massChangeDeformationBase<num_params>::setJAt() {
            ...
            Eigen::Map<const Eigen::Matrix<floatType, spatial_dimension, spatial_dimension, Eigen::RowMajor> > _At_map(
                _At.data());
        ...
        void massChangeDeformationBase<num_params>::setGammaRHS() {
            ...
            for (unsigned int i = 0; i < spatial_dimension; i++) {
                term1[spatial_dimension * i + i] = 1;
                term2[spatial_dimension * i + i] = 1;
            }
            ...
            for (unsigned int i = 0; i < spatial_dimension; i++) {
                for (unsigned int j = 0; j < spatial_dimension; j++) {
                    ...
                    term2[spatial_dimension * i + j] +=
                        (1. - _alpha) * _dt * _gammat * (*nt)[spatial_dimension * i + j];
                }
            }

            // Compute the determinant
            Eigen::Map<const Eigen::Matrix<floatType, spatial_dimension, spatial_dimension, Eigen::RowMajor> >
                term1_map(term1.data());

            Eigen::Map<const Eigen::Matrix<floatType, spatial_dimension, spatial_dimension, Eigen::RowMajor> >
                term2_map(term2.data());
        ...
        void massChangeDeformationBase<num_params>::setGammaLHS() {
            ...
            Eigen::Map<const Eigen::Matrix<floatType, spatial_dimension, spatial_dimension, Eigen::RowMajor> >
                term2_map(term2->data());
            Eigen::Map<Eigen::Matrix<floatType, spatial_dimension, spatial_dimension, Eigen::RowMajor> > invterm1_map(
                invterm1.data());
            ...
            for (unsigned int i = 0; i < spatial_dimension; i++) {
                for (unsigned int j = 0; j < spatial_dimension; j++) {
                    gammaLHS += -detTerm1 * invterm1[spatial_dimension * j + i] * _alpha * _dt *
                                (*ntp1)[spatial_dimension * i + j];

                    dGammaRHSdNtp1[spatial_dimension * i + j] +=
                        -detTerm1 * invterm1[spatial_dimension * j + i] * _alpha * _dt * (*gammatp1);
                }
            }
        ...
        void massChangeDeformationBase<num_params>::computeMassDeformation() {
            ...
            for (unsigned int i = 0; i < spatial_dimension; i++) {
                for (unsigned int j = 0; j < spatial_dimension; j++) {
                    for (unsigned int k = 0; k < spatial_dimension; k++) {
                        RHS[spatial_dimension * i + k] +=
                            TERM1[spatial_dimension * i + j] * _At[spatial_dimension * j + k];
                    }
                }
            }

            Eigen::Map<const Eigen::Matrix<floatType, spatial_dimension, spatial_dimension, Eigen::RowMajor> > LHS_map(
                LHS.data());
            Eigen::Map<Eigen::Matrix<floatType, spatial_dimension, spatial_dimension, Eigen::RowMajor> > invLHS_map(
                invLHS.data());
            ...
            for (unsigned int i = 0; i < spatial_dimension; i++) {
                for (unsigned int j = 0; j < spatial_dimension; j++) {
                    for (unsigned int k = 0; k < spatial_dimension; k++) {
                        Atp1[spatial_dimension * i + k] +=
                            invLHS[spatial_dimension * i + j] * RHS[spatial_dimension * j + k];
                    }
                }
            }
            ...
            for (unsigned int b = 0; b < spatial_dimension; b++) {
                for (unsigned int j = 0; j < spatial_dimension; j++) {
                    for (unsigned int k = 0; k < spatial_dimension; k++) {
                        TERM2[spatial_dimension * b + k] +=
                            _alpha * _dt * invLHS[spatial_dimension * b + j] * RHS[spatial_dimension * j + k];
                    }
                }
            }
            ...
            for (unsigned int i = 0; i < spatial_dimension; i++) {
                for (unsigned int k = 0; k < spatial_dimension; k++) {
                    for (unsigned int a = 0; a < spatial_dimension; a++) {
                        for (unsigned int b = 0; b < spatial_dimension; b++) {
                            dAtp1dLtp1[sot_dimension * spatial_dimension * i + sot_dimension * k +
                                       spatial_dimension * a + b] +=
            ...
            for (unsigned int i = 0; i < sot_dimension; i++) {
                for (unsigned int j = 0; j < sot_dimension; j++) {
                    for (unsigned int k = 0; k < sot_dimension; k++) {
                        dAtp1dNtp1[sot_dimension * i + k] +=
                            dAtp1dLtp1[sot_dimension * i + j] * dLtp1dNtp1[sot_dimension * j + k];
                    }
                }
            }
        ...
        massChangeWeightedDirection::massChangeWeightedDirection(
            const floatType &dt, const secondOrderTensor &At, const floatType &ct, const floatType &ctp1,
            const floatType &rhot, const floatType &rhotp1, const floatType &gammat, const vector3d &vt,
            const vector3d &vtp1, const std::array<floatType, 1> &parameters, const floatType alpha,
            const floatType tolr, const floatType tola, const unsigned int maxiter, const floatType lsalpha,
            const unsigned int maxlsiter)
             ...
             * \param &rhotp1: The current value of the mass density (units: \f$ \frac{m}{dv} \f$)
             * \param &gammat: The previous rate multiplier for the evolution of the mass-change deformation (units:
             * None) \param &parameters: The parameter vector d The weighting factor for whether the flow is volumetric
             * (0) or in a direction with an eigen-vector in the direction of \f$ \bf{v}^t \f$ and \f$ \bf{v}^{t+1} \f$
             * (1) \param &alpha: The integration parameter (0 for explicit 1 for implicit). Defaults to the
             * second-order accurate value of 0.5 \param tolr: The relative tolerance \param tola: The absolute
             * tolerance \param maxiter: The maximum number of iterations \param lsalpha: The line search alpha
             * parameter (minimum expected relative residual improvement) \param maxlsiter: The maximum number of line
             * search iterations
             */
        }
        ...
        void massChangeWeightedDirection::setNt() {
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK((1 >= _d) && (_d >= 0),
                                         "The d parameter must be between 0 and 1.\n  value: " + std::to_string(_d));
        ...
        void massChangeWeightedDirection::setNtp1() {
            ...
            for (unsigned int i = 0; i < spatial_dimension; i++) {
                ntp1[spatial_dimension * i + i] += (1 - _d);
            }
        ...
        void massChangeWeightedDirection::setdNtp1dVtp1() {
            ...
            for (unsigned int i = 0; i < spatial_dimension; i++) {
                for (unsigned int j = 0; j < spatial_dimension; j++) {
                    for (unsigned int k = 0; k < spatial_dimension; k++) {
                        dNtp1dVtp1[spatial_dimension * spatial_dimension * i + spatial_dimension * j + k] +=
                            _d * (*get_dirtp1())[j] * (*get_dDirtp1dVtp1())[spatial_dimension * i + k];

                        dNtp1dVtp1[spatial_dimension * spatial_dimension * i + spatial_dimension * j + k] +=
                            _d * (*get_dirtp1())[i] * (*get_dDirtp1dVtp1())[spatial_dimension * j + k];
                    }
                }
            }
            ...
            for (unsigned int i = 0; i < sot_dimension; i++) {
                for (unsigned int j = 0; j < sot_dimension; j++) {
                    for (unsigned int k = 0; j < spatial_dimension; j++) {
                        dAtp1dVtp1[spatial_dimension * i + k] +=
                            (*dAtp1dNtp1)[sot_dimension * i + j] * (*dNtp1dVtp1)[spatial_dimension * j + k];
                    }
                }
            }
