...
namespace tardigradeStressTools {
    namespace linearElasticity {
        ...
        void formReferenceStiffnessTensor(const floatVector &parameters, floatMatrix &stiffnessTensor) {
             ...
             * \end{bmatrix}\f$
             *
             * \param &parameters: The tensor components of the 9x9 stiffness tensor. Vector length determines the
             * symmetry.
             *
             ...
             * - 5: transversly isotropic or hexagonal \f$C_{1111}\f$, \f$C_{1122}\f$, \f$C_{1133}\f$, \f$C_{1313}\f$,
             * \f$C_{3333}\f$
            ...
            if (parameters.size() == 81) {
                ...
                throw std::runtime_error("Requires 81, 21, 9, 5, 3, or 2 parameters. Parameters length is " +
        ...
        void formReferenceStiffnessTensor(const floatMatrix &directionCosines, const floatVector &parameters,
             ...
             * \param &parameters: The tensor components of the 9x9 stiffness tensor, \f$C_{mnop}\f$. Vector length
                                    ...
                                    for (unsigned int o = 0; o < spatialDimensions; o++) {
                                        for (unsigned int p = 0; p < spatialDimensions; p++) {
                                            stiffnessTensor[(spatialDimensions * i) + j][(spatialDimensions * k) + l] +=
                                                directionCosines[i][m] * directionCosines[j][n] *
                                                directionCosines[k][o] * directionCosines[l][p] *
                                                unrotatedStiffnessTensor[(spatialDimensions * m) + n]
                                                                        [(spatialDimensions * o) + p];
                                        }
                                    }
                                }
                            }
        ...
        void formReferenceStiffnessTensor(const floatVector &bungeEulerAngles, const floatVector &parameters,
             ...
             * \f$C_{mnop}\f$ is the original stiffness tensor.
             *
             * \param &bungeEulerAngles: Vector containing three Bunge-Euler angles in radians to form the rotation
             * matrix, \f$R_{ij}\f$, using ``tardigradeVectorTools::rotationMatrix`` \param &parameters: The tensor
             * components of the 9x9 stiffness tensor, \f$C_{mnop}\f$. Vector length determines the symmetry.
             *
             * - 81: A row-major vector representing the full 9x9 stiffness tensor directly.
             * - 21: fully anistropic \f$C_{1111}\f$, \f$C_{1112}\f$, \f$C_{1113}\f$, \f$C_{1122}\f$, \f$C_{1123}\f$,
             *   \f$C_{1133}\f$, \f$C_{1212}\f$, \f$C_{1213}\f$, \f$C_{1222}\f$, \f$C_{1223}\f$, \f$C_{1233}\f$,
        ...
        void evaluateEnergy(const floatVector &chi, const floatVector &parameters, floatType &energy) {
             ...
             *
             * \f$\rho \psi = \frac{1}{J} \left[ \frac{\lambda}{2} \left( E_{II} \right)^2 + \mu E_{IJ} E_{JI} \right]
             * \f$
             *
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK(chi.size() == spatialDimensions * spatialDimensions,
                                         "The spatial dimension of " + std::to_string(spatialDimensions) +
                                             " is not reflected by chi which has a size of " +
                                             std::to_string(chi.size()) + " rather than " +
                                             std::to_string(spatialDimensions * spatialDimensions));
        ...
        void evaluateEnergy(const floatVector &chi, const floatVector &parameters, floatType &energy,
            ...
            floatVector dEnergydChi =
                tardigradeVectorTools::Tdot(dEdChi, tardigradeVectorTools::dot(C, E)) / detChi -
                energy * tardigradeVectorTools::matrixMultiply(eye, invChi, spatialDimensions, spatialDimensions,
                                                               spatialDimensions, spatialDimensions, false, true);
            ...
            for (unsigned int i = 0; i < spatialDimensions; i++) {
                for (unsigned int j = 0; j < spatialDimensions; j++) {
                    for (unsigned int I = 0; I < spatialDimensions; I++) {
                        cauchyStress[spatialDimensions * i + j] +=
                            dEnergydChi[spatialDimensions * i + I] * chi[spatialDimensions * j + I] / detChi;
                    }
                }
            }
        ...
        void evaluateEnergy(const floatVector &chi, const floatVector &parameters, floatType &energy,
                            floatVector &cauchyStress, floatVector &dEnergydChi, floatMatrix &dCauchyStressdChi) {
            /*!
             * Compute the value of the linear elastic energy which we define via
             *
             * \f$\rho \psi = \frac{1}{J} \left[ \frac{\lambda}{2} \left( E_{II} \right)^2 + \mu E_{IJ} E_{JI} \right]
            ...
            floatVector invChiT =
                tardigradeVectorTools::matrixMultiply(eye, invChi, spatialDimensions, spatialDimensions,
                                                      spatialDimensions, spatialDimensions, false, true);
            ...
            for (unsigned int k = 0; k < spatialDimensions; k++) {
                for (unsigned int K = 0; K < spatialDimensions; K++) {
                    for (unsigned int l = 0; l < spatialDimensions; l++) {
                        for (unsigned int L = 0; L < spatialDimensions; L++) {
                            d2EnergydChi2[spatialDimensions * spatialDimensions * spatialDimensions * k +
                                          spatialDimensions * spatialDimensions * K + spatialDimensions * l + L] +=
                            ...
                            for (unsigned int IJ = 0; IJ < spatialDimensions * spatialDimensions; IJ++) {
                                d2EnergydChi2[spatialDimensions * spatialDimensions * spatialDimensions * k +
                                    ...
                                    (dEdChi[IJ][spatialDimensions * k + K] * dCEdChi[IJ][spatialDimensions * l + L] -
                                     dEdChi[IJ][spatialDimensions * k + K] * CE[IJ] *
            ...
            // Use the energy gradient to compute the Cauchy stress
            cauchyStress      = floatVector(spatialDimensions * spatialDimensions, 0);
            dCauchyStressdChi = floatMatrix(spatialDimensions * spatialDimensions,
                                            floatVector(spatialDimensions * spatialDimensions, 0));
            ...
            for (unsigned int i = 0; i < spatialDimensions; i++) {
                for (unsigned int j = 0; j < spatialDimensions; j++) {
                    for (unsigned int I = 0; I < spatialDimensions; I++) {
                        ...
                        for (unsigned int k = 0; k < spatialDimensions; k++) {
                            for (unsigned int K = 0; K < spatialDimensions; K++) {
                                dCauchyStressdChi[spatialDimensions * i + j][spatialDimensions * k + K] +=
                                    (d2EnergydChi2[spatialDimensions * spatialDimensions * spatialDimensions * i +
                                                   spatialDimensions * spatialDimensions * I + spatialDimensions * k +
                                                   K] *
                                         chi[spatialDimensions * j + I] +
                                     dEnergydChi[spatialDimensions * i + I] * eye[spatialDimensions * j + k] *
                                         eye[spatialDimensions * I + K] -
                                     dEnergydChi[spatialDimensions * i + I] * chi[spatialDimensions * j + I] *
                                         invChi[spatialDimensions * K + k]) /
                                    detChi;
                            }
                        }
                    }
        ...
        void evaluateEnergy(const floatVector &chi, const floatVector &parameters, floatType &energy,
            ...
            d2EnergydChi2 =
                floatVector(spatialDimensions * spatialDimensions * spatialDimensions * spatialDimensions, 0);

            floatVector d3EnergydChi3(spatialDimensions * spatialDimensions * spatialDimensions * spatialDimensions *
                ...
                for (unsigned int K = 0; K < spatialDimensions; K++) {
                    for (unsigned int l = 0; l < spatialDimensions; l++) {
                        for (unsigned int L = 0; L < spatialDimensions; L++) {
                            for (unsigned int m = 0; m < spatialDimensions; m++) {
                                for (unsigned int M = 0; M < spatialDimensions; M++) {
                                    d3EnergydChi3[spatialDimensions * spatialDimensions * spatialDimensions *
                                                      spatialDimensions * spatialDimensions * k +
                                                  spatialDimensions * spatialDimensions * spatialDimensions *
                                                      spatialDimensions * K +
                                                  spatialDimensions * spatialDimensions * spatialDimensions * l +
                                                  spatialDimensions * spatialDimensions * L + spatialDimensions * m +
                                                  M] +=
                                        (eye[spatialDimensions * l + k] *
                                             dCEdChi[spatialDimensions * K + L][spatialDimensions * m + M] -
                                         eye[spatialDimensions * l + k] * CE[spatialDimensions * K + L] *
                                             invChi[spatialDimensions * M + m]) /
                                            detChi -
                                        d2EnergydChi2[spatialDimensions * spatialDimensions * spatialDimensions * l +
                                                      spatialDimensions * spatialDimensions * L +
                                                      spatialDimensions * m + M] *
                                            invChi[spatialDimensions * K + k] +
                                        dEnergydChi[spatialDimensions * l + L] * invChi[spatialDimensions * K + m] *
                                            invChi[spatialDimensions * M + k] +
                                        dEnergydChi[spatialDimensions * m + M] * invChi[spatialDimensions * K + l] *
                                            invChi[spatialDimensions * L + k] -
                                        energy * invChi[spatialDimensions * K + m] * invChi[spatialDimensions * M + l] *
                                            invChi[spatialDimensions * L + k] -
                                        energy * invChi[spatialDimensions * K + l] * invChi[spatialDimensions * L + m] *
                                            invChi[spatialDimensions * M + k] +
                                        (eye[spatialDimensions * m + k] *
                                             dCEdChi[spatialDimensions * K + M][spatialDimensions * l + L] -
                                         eye[spatialDimensions * m + k] * CE[spatialDimensions * K + M] *
                                             invChi[spatialDimensions * L + l]) /
                                            detChi;
                                    for (unsigned int I = 0; I < spatialDimensions; I++) {
                                        for (unsigned int J = 0; J < spatialDimensions; J++) {
                                            d3EnergydChi3[spatialDimensions * spatialDimensions * spatialDimensions *
                                                          ...
                                                          spatialDimensions * spatialDimensions * L +
                                                          spatialDimensions * m + M] +=
                                                (+dEdChi[spatialDimensions * I + J][spatialDimensions * k + K] *
                                                     C[spatialDimensions * I + J][spatialDimensions * L + M] *
                                                     eye[spatialDimensions * m + l] -
                                                 dEdChi[spatialDimensions * I + J][spatialDimensions * k + K] *
                                                     dCEdChi[spatialDimensions * I + J][spatialDimensions * m + M] *
                                                     invChi[spatialDimensions * L + l] +
                                                 dEdChi[spatialDimensions * I + J][spatialDimensions * k + K] *
                                                     CE[spatialDimensions * I + J] * invChi[spatialDimensions * L + m] *
                                                     invChi[spatialDimensions * M + l] -
                                                 dEdChi[spatialDimensions * I + J][spatialDimensions * k + K] *
                                                     dCEdChi[spatialDimensions * I + J][spatialDimensions * l + L] *
                                                     invChi[spatialDimensions * M + m] +
                                                 dEdChi[spatialDimensions * I + J][spatialDimensions * k + K] *
                                                     CE[spatialDimensions * I + J] * invChi[spatialDimensions * L + l] *
                                                     invChi[spatialDimensions * M + m]) /
                                                detChi;
                                        }
                                    }
            ...
            dCauchyStressdChi = floatMatrix(spatialDimensions * spatialDimensions,
                                            floatVector(spatialDimensions * spatialDimensions, 0));
            d2CauchyStressdChi2 =
                floatMatrix(spatialDimensions * spatialDimensions,
                            floatVector(spatialDimensions * spatialDimensions * spatialDimensions * spatialDimensions,
                                        0));
                    ...
                    for (unsigned int I = 0; I < spatialDimensions; I++) {
                        ...
                        for (unsigned int k = 0; k < spatialDimensions; k++) {
                            for (unsigned int K = 0; K < spatialDimensions; K++) {
                                ...
                                for (unsigned int l = 0; l < spatialDimensions; l++) {
                                    for (unsigned int L = 0; L < spatialDimensions; L++) {
                                        d2CauchyStressdChi2[spatialDimensions * i + j]
                                                           [spatialDimensions * spatialDimensions * spatialDimensions *
                                                                k +
                                                            spatialDimensions * spatialDimensions * K +
                                                            spatialDimensions * l + L] +=
                                            (d3EnergydChi3[spatialDimensions * spatialDimensions * spatialDimensions *
                                                               spatialDimensions * spatialDimensions * i +
                                                           spatialDimensions * spatialDimensions * spatialDimensions *
                                                               spatialDimensions * I +
                                                           spatialDimensions * spatialDimensions * spatialDimensions *
                                                               k +
                                                           ...
                                                           spatialDimensions * l + L] *
                                                 chi[spatialDimensions * j + I] +
                                             d2EnergydChi2[spatialDimensions * spatialDimensions * spatialDimensions *
                                                               i +
                                                           spatialDimensions * spatialDimensions * I +
                                                           spatialDimensions * k + K] *
                                                 eye[spatialDimensions * j + l] * eye[spatialDimensions * I + L] +
                                             d2EnergydChi2[spatialDimensions * spatialDimensions * spatialDimensions *
                                                               i +
                                                           spatialDimensions * spatialDimensions * I +
                                                           spatialDimensions * l + L] *
                                                 eye[spatialDimensions * j + k] * eye[spatialDimensions * I + K] -
                                             d2EnergydChi2[spatialDimensions * spatialDimensions * spatialDimensions *
                                                               i +
                                                           spatialDimensions * spatialDimensions * I +
                                                           spatialDimensions * l + L] *
                                                 chi[spatialDimensions * j + I] * invChi[spatialDimensions * K + k] -
                                             dEnergydChi[spatialDimensions * i + I] * eye[spatialDimensions * j + l] *
                                                 eye[spatialDimensions * I + L] * invChi[spatialDimensions * K + k] +
                                             dEnergydChi[spatialDimensions * i + I] * chi[spatialDimensions * j + I] *
                                                 invChi[spatialDimensions * K + l] * invChi[spatialDimensions * L + k] -
                                             (d2EnergydChi2[spatialDimensions * spatialDimensions * spatialDimensions *
                                                                i +
                                                            spatialDimensions * spatialDimensions * I +
                                                            spatialDimensions * k + K] *
        ...
        void evaluateEnergy(const floatVector &bungeEulerAngles, const floatVector &chi, const floatVector &parameters,
                            floatType &energy, floatVector &cauchyStress, floatVector &dEnergydChi,
                            floatMatrix &dCauchyStressdChi, floatVector &d2EnergydChi2,
                            floatMatrix &d2CauchyStressdChi2) {
            /*!
             * Compute the value of the linear elastic energy which we define via
             *
             * \f$\rho \psi = \frac{1}{J} \left[ \frac{\lambda}{2} \left( E_{II} \right)^2 + \mu E_{IJ} E_{JI} \right]
             ...
             *
             * \param &bungeEulerAngles: Vector containing three Bunge-Euler angles in radians to form the rotation
             * matrix, \f$R_{ij}\f$, using ``tardigradeVectorTools::rotationMatrix`` \param &chi: The micro-deformation
             * \param &parameters: The tensor components of the 9x9 stiffness tensor. Vector length determines the
             * symmetry.
             *
             * - 81: A row-major vector representing the full 9x9 stiffness tensor directly.
            ...
            TARDIGRADE_ERROR_TOOLS_CATCH(evaluateEnergy(chi, flatStiffnessTensor, energy, cauchyStress, dEnergydChi,
                                                        dCauchyStressdChi, d2EnergydChi2, d2CauchyStressdChi2))
