...
BOOST_AUTO_TEST_CASE(testCalculateMeanStress, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    ...
    floatVector stressVector = {1., 0., 0., 0., 1., 0., 0., 0., 1.};
    floatMatrix stressMatrix = {
        {1., 0., 0.},
        {0., 1., 0.},
        {0., 0., 1.}
    };
    floatType   meanStressExpected     = 1.;
    floatVector jacobianVectorExpected = {1. / 3., 0., 0., 0., 1. / 3., 0., 0., 0., 1. / 3.};
...
BOOST_AUTO_TEST_CASE(testCalculateDeviatoricStress, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
     ...
     */
    floatVector stressVector   = {1., 0., 0., 0., 1., 0., 0., 0., 1.};
    floatVector expectedVector = {0., 0., 0., 0., 0., 0., 0., 0., 0.};
    ...
    BOOST_TEST(tardigradeVectorTools::appendVectors(jacobian) == tardigradeVectorTools::appendVectors(jacobian2),
               CHECK_PER_ELEMENT);
}
...
BOOST_AUTO_TEST_CASE(testCalculateVonMisesStress, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    ...
    // Initialize test values
    floatVector stressVector           = {1., 1., 1., 1., 1., 1., 1., 1., 1.};
    floatVector jacobianVectorExpected = {0., 1. / 2., 1. / 2., 1. / 2., 0., 1. / 2., 1. / 2., 1. / 2., 0.};
...
BOOST_AUTO_TEST_CASE(testCalculateVonMisesStress2, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    /*!
     * Test the von Mises stress calculation
     */

    // Initialize test values
    floatVector stressVector           = {0., 0., 0., 0., 0., 0., 0., 0., 0.};
    floatVector jacobianVectorExpected = {0., 0., 0., 0., 0., 0., 0., 0., 0.};
    floatType   vonMisesExpected       = 0.0;
...
BOOST_AUTO_TEST_CASE(testCalculateVonMisesStress3, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    /*!
     * Test the von Mises stress calculation
     */
...

    {
        ...
        constexpr unsigned int OUT_DIM = 1;
        floatVector            X       = stressVector;
        ...
        for (unsigned int i = 0; i < VAR_DIM; ++i) {
            ...
            floatVector rp = {tardigradeStressTools::calculateVonMisesStress(Xp)};
            floatVector rm = {tardigradeStressTools::calculateVonMisesStress(Xm)};

            for (unsigned int j = 0; j < OUT_DIM; ++j) {
                BOOST_TEST(jacobianVector[9 * j + i] == (rp[j] - rm[j]) / (2 * delta));
            }
        }
    }
}

BOOST_AUTO_TEST_CASE(testCalculateVonMisesStress4, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    /*!
     * Test the von Mises stress calculation
     */
...
BOOST_AUTO_TEST_CASE(testDruckerPragerSurface, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    ...
    floatVector jacobianVectorExpected      = {1. / 3., 1. / 2., 1. / 2., 1. / 2., 1. / 3.,
                                               1. / 2., 1. / 2., 1. / 2., 1. / 3.};
    floatVector unitDirectionVectorExpected = {1. / 3., 1. / 2., 1. / 2., 1. / 2., 1. / 3.,
                                               1. / 2., 1. / 2., 1. / 2., 1. / 3.};
...
BOOST_AUTO_TEST_CASE(testDruckerPragerSurface2, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    /*!
     * Test the Drucker-Prager yield criterion calculation.
     */
    ...
    floatType   dpYieldExpected             = 0.;
    floatVector jacobianVectorExpected      = {1. / 3., 0., 0., 0., 1. / 3., 0., 0., 0., 1. / 3.};
    floatVector unitDirectionVectorExpected = {1. / 3., 0., 0., 0., 1. / 3., 0., 0., 0., 1. / 3.};
    unitDirectionVectorExpected /= sqrt(1. / 3);
    ...
    BOOST_TEST(dpYield == dpYieldExpected);
    BOOST_TEST(jacobianVector == jacobianVectorExpected, CHECK_PER_ELEMENT);
    BOOST_TEST(tardigradeVectorTools::appendVectors(djacobiandstress) == floatVector(81, 0), CHECK_PER_ELEMENT);
    ...
    BOOST_TEST(jacobianVector == jacobianVectorExpected, CHECK_PER_ELEMENT);
    BOOST_TEST(unitDirectionVector == unitDirectionVectorExpected, CHECK_PER_ELEMENT);
    BOOST_TEST(tardigradeVectorTools::appendVectors(unitDirectionJacobian) == floatVector(81, 0), CHECK_PER_ELEMENT);
...
BOOST_AUTO_TEST_CASE(testLinearViscoelasticity, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    ...
    floatVector previousStrain = {3.03768940e-01,  4.54626930e-17,  -3.71231060e-01, 4.54626930e-17, 2.00000000e-01,
                                  -1.03633416e-16, -3.71231060e-01, -1.03633416e-16, 1.04623106e+00};

    floatVector currentStrain = {1.0163119,   -0.57654737, 0.33286978,  -0.57654737, 0.45526608,
                                 -0.22243347, 0.33286978,  -0.22243347, 0.19842203};

    floatVector previousStateVariables = {1, 2, 3, 2, 4, 5, 3, 5, 6, 1.0, 0.3, 0.2, 0.3, 2.0, 0.1, 0.2, 0.1, 3.0};
    ...
    floatVector previousStress =
        materialParameters[0] * previousStrain +
        materialParameters[3] *
            (previousStrain - floatVector(previousStateVariables.begin(), previousStateVariables.begin() + 9)) +
        materialParameters[4] *
            (previousStrain - floatVector(previousStateVariables.begin() + 9, previousStateVariables.begin() + 18));
    ...
    tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
    ...
    tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                 currentRateModifier, previousRateModifier, previousStateVariables,
                                                 materialParameters, 0., stress, currentStateVariables);
    ...
    for (unsigned int i = 0; i < previousStateVariables.size() / dim; i++) {
        ...
        rotatedPreviousStateVariables =
            tardigradeVectorTools::appendVectors({rotatedPreviousStateVariables, rotatedSubv});
    }
    ...
    tardigradeStressTools::linearViscoelasticity(currentTime, rotatedCurrentStrain, previousTime, rotatedPreviousStrain,
                                                 currentRateModifier, previousRateModifier,
                                                 rotatedPreviousStateVariables, materialParameters, alpha,
    ...
    tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                 currentRateModifier, previousRateModifier, previousStateVariables,
                                                 materialParameters, alpha, stressJ, currentStateVariablesJ, jacobian,
    ...
    tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                 currentRateModifier, previousRateModifier, previousStateVariables,
                                                 materialParameters, alpha, dStress_2, stress_2,
                                                 currentStateVariables_2, jacobian_2, dstressdrateModifier_2);
    ...
    BOOST_TEST(tardigradeVectorTools::appendVectors(jacobian) == tardigradeVectorTools::appendVectors(jacobian_2),
               CHECK_PER_ELEMENT);
    ...
    floatMatrix jacobian_3, dstressdPreviousStrain, dstressdPreviousStateVariables, dStateVariablesdStrain,
        dStateVariablesdPreviousStrain, dStateVariablesdPreviousStateVariables;

    tardigradeStressTools::linearViscoelasticity(
        currentTime, currentStrain, previousTime, previousStrain, currentRateModifier, previousRateModifier,
        previousStateVariables, materialParameters, alpha, dStress_3, stress_3, currentStateVariables_3, jacobian_3,
        dstressdrateModifier_3, dstressdPreviousStrain, dstressdPreviousRateModifier, dstressdPreviousStateVariables,
        dStateVariablesdStrain, dStateVariablesdRateModifier, dStateVariablesdPreviousStrain,
    ...
    BOOST_TEST(tardigradeVectorTools::appendVectors(jacobian_3) == tardigradeVectorTools::appendVectors(jacobian),
               CHECK_PER_ELEMENT);
    ...
    floatMatrix dStateVariablesdPreviousStrain_answer(currentStateVariables.size(),
                                                      floatVector(previousStrain.size(), 0));
    floatVector dStateVariablesdPreviousRateModifier_answer(currentStateVariables.size(), 0);
    floatMatrix dStateVariablesdPreviousStateVariables_answer(currentStateVariables.size(),
                                                              floatVector(previousStateVariables.size(), 0));
    ...
    for (unsigned int i = 0; i < currentStrain.size(); i++) {
        ...
        tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain + delta, previousTime, previousStrain,
                                                     currentRateModifier, previousRateModifier, previousStateVariables,
                                                     materialParameters, alpha, s_p, xi_p);

        tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain - delta, previousTime, previousStrain,
                                                     currentRateModifier, previousRateModifier, previousStateVariables,
                                                     materialParameters, alpha, s_m, xi_m);
    ...
    BOOST_TEST(tardigradeVectorTools::appendVectors(jacobian) == tardigradeVectorTools::appendVectors(jacobian_answer),
               CHECK_PER_ELEMENT);

    BOOST_TEST(tardigradeVectorTools::appendVectors(dStateVariablesdStrain) ==
                   tardigradeVectorTools::appendVectors(dStateVariablesdStrain_answer),
               CHECK_PER_ELEMENT);
    ...
    for (unsigned int i = 0; i < 1; i++) {
        ...
        tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                     ...
                                                     previousStateVariables, materialParameters, alpha, s_p, xi_p);

        tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                     currentRateModifier - delta, previousRateModifier,
                                                     previousStateVariables, materialParameters, alpha, s_m, xi_m);
    ...
    for (unsigned int i = 0; i < previousStrain.size(); i++) {
        ...
        tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain + delta,
                                                     currentRateModifier, previousRateModifier, previousStateVariables,
                                                     materialParameters, alpha, s_p, xi_p);
    ...
    BOOST_TEST(tardigradeVectorTools::appendVectors(dstressdPreviousStrain) ==
                   tardigradeVectorTools::appendVectors(dstressdPreviousStrain_answer),
               CHECK_PER_ELEMENT);

    BOOST_TEST(tardigradeVectorTools::appendVectors(dStateVariablesdPreviousStrain) ==
                   tardigradeVectorTools::appendVectors(dStateVariablesdPreviousStrain_answer),
               CHECK_PER_ELEMENT);
    ...
    for (unsigned int i = 0; i < previousStateVariables.size(); i++) {
        ...
        tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                     currentRateModifier, previousRateModifier,
                                                     previousStateVariables + delta, materialParameters, alpha, s_p,
                                                     xi_p);

        tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                     currentRateModifier, previousRateModifier,
                                                     previousStateVariables - delta, materialParameters, alpha, s_m,
                                                     xi_m);
    ...
    BOOST_TEST(tardigradeVectorTools::appendVectors(dstressdPreviousStateVariables) ==
                   tardigradeVectorTools::appendVectors(dstressdPreviousStateVariables_answer),
               CHECK_PER_ELEMENT);

    BOOST_TEST(tardigradeVectorTools::appendVectors(dStateVariablesdPreviousStateVariables) ==
                   tardigradeVectorTools::appendVectors(dStateVariablesdPreviousStateVariables_answer),
               CHECK_PER_ELEMENT);
    ...
    tardigradeStressTools::linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                 currentRateModifier, previousRateModifier, previousStateVariablesOdd,
                                                 materialParametersOdd, alpha, stress, currentStateVariables);
}
...
BOOST_AUTO_TEST_CASE(testVolumetricNeoHookean, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    ...
    deformationGradient = {0.39874077, 0.11561812, -0.75485222, 0.14034205, 0.15851022,
    ...
    deformationGradient = {-0.2350804,  0.16410551, -1.13402371, 0.1296644, -0.22975865,
                           -1.03460443, -0.4188584, -0.16322821, 0.31618178};
...
BOOST_AUTO_TEST_CASE(testComputeJaumannStiffnessTensor, *boost::unit_test::tolerance(DEFAULT_TEST_TOLERANCE)) {
    ...
    floatVector cauchyStress = {0.39293837,  -0.42772133, -0.54629709, 0.10262954, 0.43893794,
                                -0.15378708, 0.9615284,   0.36965948,  -0.0381362};

    floatVector currentDeformationGradient = {0.89211752,  -0.15682198, 0.22904971,  -0.06142776, 0.5596779,
                                              -0.10195574, 0.23799541,  -0.31750827, 0.67545176};

    floatMatrix dCauchydF = {
        {0.3155137384183837,  0.3182758709686606,  1.3440095855132106,  3.4943179407778957,   2.244553248606352,
         1.1102351067758287,  2.2244338257022154,   -1.770410861468218,  -1.382113443776859 },
        {-2.717367691210444,  -2.0628595361117066, 1.3097612385448776,  -4.078950600549248,   -0.6629882732047176,
         -0.6913723667035621, -0.06314902349693785, -0.7416970970417203, -1.8773877702753472},
        {-0.7364869303719179, 3.933891631171348,   4.441600182038796,   0.018366758843365583, 1.2395295179211119,
         -3.843816049207043,  -1.827145181796791,   -0.8517378804636822, 3.6630915788336593 },
        {-2.495446346034933,  -0.1696573573729565, 4.85559785610705,    0.19485119259809336,  1.1289452576296766,
         -3.7937133400967626, 3.2634080050683325,   1.0306012841092738,  0.45068006466464916},
        {-1.5723616622569159, -1.958792109728159,  -0.8297778897529839, 1.8130076579279664,   3.754568417951749,
         0.10422337478011134, 1.6931378296227229,   0.8593655256221289,  1.2490350209559986 },
        {1.7468905098782483,  3.423424376202573,   -4.168050116675612,  2.6368284144333822,   -2.5633362546312597,
         -3.0577703942122914, 0.7245695749147307,   -4.0428748338761284, 3.8532682627513957 },
        {1.2724897205126873,  2.234163581899548,   -4.8387079330498315, 0.944318794450425,    0.567851923942887,
         -3.4104035585527726, -3.469294848752269,   1.955295287709109,   -1.8123357361812364},
        {1.9197029553181966,  0.5438324971777209,  -1.110494258768554,  4.251324896139861,    3.416699969127163,
         -1.4260243331682376, -4.564085362009594,   -1.9523192658890254, -1.0181431808201902},
        {2.0495883045136223,  4.953584820340175,   -1.4408513428254044, 2.625478137854338,    0.9317691656222116,
         1.9170179870017712,  -3.488725476519198,   -1.0112370727384346, -2.591441022763755 }
    };

    floatMatrix CAnswer = {
        {0.9323460532562229,   1.5206835766443023,  1.413686140800323,   1.5206835766443023,  1.3213222576565693,
         -0.05885487383212795, 1.413686140800323,   -0.05885487383212795, 0.5509125382847507 },
        {-2.2284304975160723,  -2.4072217797743627, 0.2614487023857131,  -2.4072217797743627, -0.47773104837760527,
         -0.7235389116516223,  0.2614487023857131,  -0.7235389116516223,  -1.4753404121609062},
        {-0.8029034532123083,  0.36784058323148006, 0.45916803313893,    0.36784058323148006, 0.5382110794054509,
         -1.8617183183800223,  0.45916803313893,    -1.8617183183800223,  1.7635262086408587 },
        {-0.9848225913718358,  -0.6544410518218504, 2.796317798370991,   -0.6544410518218504, 1.1092968426129497,
         -1.2720772977623038,  2.796317798370991,   -1.2720772977623038,  0.8564938612296599 },
        {-0.8466721625112175,  0.06869199010472715, 0.6745222632776061,  0.06869199010472715, 2.418291731814228,
         -0.2203026144049039,  0.6745222632776061,  -0.2203026144049039,  1.4127042017386933 },
        {-0.08691431534171057, 2.1438154641991627,  -0.6617637870366262, 2.1438154641991627,  -1.4386469161393038,
         -1.6620110384171627,  -0.6617637870366262, -1.6620110384171627,  3.9050001576917728 },
        {0.6380681668217065,   0.8189097972242327,  -3.7457997213743823, 0.8189097972242327,  1.5690454177333444,
         -0.38344631567380594, -3.7457997213743823, -0.38344631567380594, -1.7091156173490465},
        {1.74261683722654,     1.6149532727880505,  -2.2323077259418067, 1.6149532727880505,  2.166152956534386,
         -0.8723741082663425,  -2.2323077259418067, -0.8723741082663425,  -0.7844009581372655},
        {0.6834798573624237,   2.714309478004904,   -2.8027874159759896, 2.714309478004904,   0.12662618464897926,
         0.7682051607516402,   -2.8027874159759896, 0.7682051607516402,   -2.2977540890151382}
    };
    ...
    BOOST_TEST(tardigradeVectorTools::appendVectors(CResult) == tardigradeVectorTools::appendVectors(CAnswer),
               CHECK_PER_ELEMENT);
}
