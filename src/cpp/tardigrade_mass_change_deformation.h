...
#define TARDIGRADE_MASS_CHANGE_DECLARE_NAMED_STORAGE(context, setname, getname, varname, vartype, setfun, uncall) \
   private:                                                                                                       \
    dataStorage<vartype> _##varname;                                                                              \
                                                                                                                  \
   public:                                                                                                        \
    TARDIGRADE_MASS_CHANGE_DECLARE_NAMED_SETTER(setname, varname, vartype, setfun)                                \
   public:                                                                                                        \
    TARDIGRADE_MASS_CHANGE_DECLARE_NAMED_GETTER(getname, varname, vartype, uncall)                                \
    context:
...
#define TARDIGRADE_MASS_CHANGE_DECLARE_STORAGE(context, varname, vartype, setfun, uncall)                         \
    TARDIGRADE_MASS_CHANGE_DECLARE_NAMED_STORAGE(context, set_##varname, get_##varname, varname, vartype, setfun, \
                                                 uncall)
...
namespace tardigradeStressTools {
    ...
    namespace massChangeDeformation {
        ...
        constexpr unsigned int sot_dimension =
            spatial_dimension * spatial_dimension;  //!< The dimension of a spatial second order tensor
        constexpr unsigned int tot_dimension =
            sot_dimension * spatial_dimension;  //!< The dimension of a spatial third order tensor
        constexpr unsigned int fot_dimension =
            tot_dimension * spatial_dimension;  //!< The dimension of a spatial fourth order tensor
        ...
        class massChangeDeformationBase {
           public:
            massChangeDeformationBase(const floatType &dt, const secondOrderTensor &At, const floatType &ct,
                                      const floatType &ctp1, const floatType &rhot, const floatType &rhotp1,
                                      const floatType &gammat, const std::array<floatType, num_params> &parameters,
                                      const floatType alpha = 0.5, const floatType tolr = 1e-9,
                                      const floatType tola = 1e-9, const unsigned int maxiter = 20,
            ...
            const std::array<floatType, num_params> *get_parameters() {
                return &_parameters;
            }  //!< Get a reference to the value of parameters

            void set_gammatp1(const floatType &val) {
                _gammatp1.second = val;
                _gammatp1.first  = true;
            }  //!< Set the value of gammatp1

            const floatType *get_gammatp1() {
                if (!_gammatp1.first) {
                    set_gammatp1(0);
                }
                return &_gammatp1.second;
            }  //!< Get a reference to the value of gammatp1
           ...
           protected:
            ...
            const floatType _lsalpha;  //!< The alpha parameter for the line search algorithm describing the minimum
                                       //!< allowable reduction in the residual
           ...
           private:
            ...
            TARDIGRADE_MASS_CHANGE_DECLARE_ITERATION_STORAGE(private, dGammaRHSdNtp1, secondOrderTensor,
                                                             setdGammaRHSdNtp1)
            ...
            TARDIGRADE_MASS_CHANGE_DECLARE_CONSTANT_STORAGE(private, dGammatp1dNtp1, secondOrderTensor,
                                                            setdGammatp1dNtp1)
            ...
            std::array<dataBase *, _num_iteration_storage>
                _iterationData;  //!< A vector of pointers to data which should be cleared at each iteration. This must
                                 //!< be the same as the number of times itertion storage is defined
        };

        //! Class for the calculation of the deformation associated with a change in mass where the volume change is
        //! weighted between a volumetric expansion and
        class massChangeWeightedDirection : public massChangeDeformationBase<1> {
           public:
            massChangeWeightedDirection(const floatType &dt, const secondOrderTensor &At, const floatType &ct,
                                        const floatType &ctp1, const floatType &rhot, const floatType &rhotp1,
                                        const floatType &gammat, const vector3d &vt, const vector3d &vtp1,
                                        const std::array<floatType, 1> &parameters, const floatType alpha = 0.5,
                                        const floatType tolr = 1e-9, const floatType tola = 1e-9,
                                        const unsigned int maxiter = 20, const floatType lsalpha = 1e-4,
                                        const unsigned int maxlsiter = 5);
