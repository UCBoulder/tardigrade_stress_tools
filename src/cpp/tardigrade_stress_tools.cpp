...
namespace tardigradeStressTools {
    ...
    void calculateMeanStress(const floatVector &stress, floatType &meanStress) {
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK((dim == 3) || (dim == 2) || (dim == 1), "The dimension of the stress is " +
                                                                                 std::to_string(dim) +
                                                                                 " but must be 1, 2, or 3");

        if (dim == 3) {
            calculateMeanStress<3>(std::begin(stress), std::end(stress), meanStress);
        } else if (dim == 2) {
            calculateMeanStress<2>(std::begin(stress), std::end(stress), meanStress);
        } else if (dim == 1) {
            calculateMeanStress<1>(std::begin(stress), std::end(stress), meanStress);
        }
    ...
    floatType calculateMeanStress(const floatVector &stress) {
        ...
        if (dim == 3) {
            return calculateMeanStress<3, floatType>(std::begin(stress), std::end(stress));
        } else if (dim == 2) {
            return calculateMeanStress<2, floatType>(std::begin(stress), std::end(stress));
        } else if (dim == 1) {
            return calculateMeanStress<1, floatType>(std::begin(stress), std::end(stress));
        }
    ...
    void calculateMeanStress(const floatMatrix &stress, floatType &meanStress) {
        ...
        if (dim == 3) {
            return calculateMeanStress<3>(std::begin(_stress), std::end(_stress), meanStress);
        } else if (dim == 2) {
            return calculateMeanStress<2>(std::begin(_stress), std::end(_stress), meanStress);
        } else if (dim == 1) {
            return calculateMeanStress<1>(std::begin(_stress), std::end(_stress), meanStress);
        }
    }
    ...
    void calculateMeanStress(const floatVector &stress, floatType &meanStress, floatVector &jacobian) {
        ...
        if (dim == 3) {
            calculateMeanStress<3>(std::begin(stress), std::end(stress), meanStress, std::begin(jacobian),
                                   std::end(jacobian));
        } else if (dim == 2) {
            calculateMeanStress<2>(std::begin(stress), std::end(stress), meanStress, std::begin(jacobian),
                                   std::end(jacobian));
        } else if (dim == 1) {
            calculateMeanStress<1>(std::begin(stress), std::end(stress), meanStress, std::begin(jacobian),
                                   std::end(jacobian));
        }
    ...
    void calculateDeviatoricStress(const floatVector &stress, floatVector &deviatoric) {
        ...
        if (dim == 3) {
            calculateDeviatoricStress<3>(std::begin(stress), std::end(stress), std::begin(deviatoric),
                                         std::end(deviatoric));
        } else if (dim == 2) {
            calculateDeviatoricStress<2>(std::begin(stress), std::end(stress), std::begin(deviatoric),
                                         std::end(deviatoric));
        } else if (dim == 1) {
            calculateDeviatoricStress<1>(std::begin(stress), std::end(stress), std::begin(deviatoric),
                                         std::end(deviatoric));
        }
    ...
    void calculateDeviatoricStress(const floatVector &stress, floatVector &deviatoric, floatVector &jacobian) {
         ...
         *
         * \f$\frac{ \partial \sigma_{ ij }^{ deviatoric } }{ \partial \sigma_{ kl } } = \delta_{ ik }\delta{ jl } -
         * \frac{ \partial \bar{ \sigma } }{ \partial \sigma_{ kl } } \delta_{ ij }\f$
         *
         * \param &stress: The stress tensor in row major format
         * \param &deviatoric: The deviatoric stress tensor in row major format
        ...
        if (dim == 3) {
            calculateDeviatoricStress<3>(std::begin(stress), std::end(stress), std::begin(deviatoric),
                                         std::end(deviatoric), std::begin(jacobian), std::end(jacobian));
        } else if (dim == 2) {
            calculateDeviatoricStress<2>(std::begin(stress), std::end(stress), std::begin(deviatoric),
                                         std::end(deviatoric), std::begin(jacobian), std::end(jacobian));
        } else if (dim == 1) {
    ...
    void calculateVonMisesStress(const floatVector &stress, floatType &vonMises) {
        ...
        if (dim == 3) {
            calculateVonMisesStress<3>(std::begin(stress), std::end(stress), vonMises);
        } else if (dim == 2) {
            calculateVonMisesStress<2>(std::begin(stress), std::end(stress), vonMises);
        } else if (dim == 1) {
            calculateVonMisesStress<1>(std::begin(stress), std::end(stress), vonMises);
        }
    ...
    void calculateVonMisesStress(const floatVector &stress, floatType &vonMises, floatVector &jacobian) {
        ...
        if (dim == 3) {
            calculateVonMisesStress<3>(std::begin(stress), std::end(stress), vonMises, std::begin(jacobian),
                                       std::end(jacobian));
        } else if (dim == 2) {
            calculateVonMisesStress<2>(std::begin(stress), std::end(stress), vonMises, std::begin(jacobian),
                                       std::end(jacobian));
        } else if (dim == 1) {
            calculateVonMisesStress<1>(std::begin(stress), std::end(stress), vonMises, std::begin(jacobian),
                                       std::end(jacobian));
        }
    ...
    void druckerPragerSurface(const floatType &vonMises, const floatType &meanStress, const floatVector &dpParam,
                              floatType &dpYield) {
        /*!
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(druckerPragerSurface_iter(vonMises, meanStress, std::begin(dpParam),
                                                               std::end(dpParam), dpYield))
    ...
    void druckerPragerSurface(const floatVector &stress, const floatVector &dpParam, floatType &dpYield,
        ...
        // Check Drucker-Prager parameter vector length
        TARDIGRADE_ERROR_TOOLS_CHECK(dpParam.size() == 2,
                                     "Two parameters are required for the Drucker-Prager surface.");
    ...
    void druckerPragerSurfaceFlatJ(const floatVector &stress, const floatType &A, const floatType &B,
                                   floatType &dpYield, floatVector &jacobian, floatVector &djacobiandstress,
                                   const floatType tol) {
        /*!
         * Compute the Drucker-Prager yield criterion from a 2nd rank stress tensor stored in row major format
         *
        ...
        for (unsigned int ij = 0; ij < rows; ++ij) {
            ...
            for (unsigned int ab = 0; ab < cols; ++ab) {
                djacobiandstress[cols * ij + ab] -=
                    (3 / (2 * (vonMises + tol))) *
                    (eye[ij] * meanStressJacobian[ab] + deviatoric[ij] * vonMisesJacobian[ab] / (vonMises + tol));
            }
        }
    ...
    void druckerPragerSurface(const floatVector &stress, const floatType &A, const floatType &B, floatType &dpYield,
                              floatVector &jacobian, floatMatrix &djacobiandstress, const floatType tol) {
        /*!
         * Compute the Drucker-Prager yield criterion from a 2nd rank stress tensor stored in row major format
         *
         * \f$f = \sigma^{ vonMises } + A*\sigma^{ mean } - B\f$
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(druckerPragerSurfaceFlatJ(stress, A, B, dpYield, jacobian, _djacobiandstress,
                                                               tol););

        djacobiandstress = tardigradeVectorTools::inflate(_djacobiandstress, jacobian.size(), stress.size());
    }

    void druckerPragerSurfaceFlatJ(const floatVector &stress, const floatVector &dpParam, floatType &dpYield,
                                   floatVector &jacobian, floatVector &djacobiandstress) {
        /*!
         * Compute the Drucker-Prager yield criterion from a 2nd rank stress tensor stored in row major format
         *
         * \f$f = \sigma^{ vonMises } + A*\sigma^{ mean } - B\f$
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(druckerPragerSurfaceFlatJ(stress, dpParam[0], dpParam[1], dpYield, jacobian,
                                                               djacobiandstress))
    ...
    void druckerPragerSurface(const floatVector &stress, const floatVector &dpParam, floatType &dpYield,
                              floatVector &jacobian, floatMatrix &djacobiandstress) {
        /*!
         * Compute the Drucker-Prager yield criterion from a 2nd rank stress tensor stored in row major format
         *
         * \f$f = \sigma^{ vonMises } + A*\sigma^{ mean } - B\f$
    ...
    void druckerPragerSurface(const floatVector &stress, const floatType &A, const floatType &B, floatType &dpYield,
                              floatVector &jacobian, floatVector &unitDirection, const floatType tol) {
        /*!
         * Compute the Drucker-Prager yield criterion from a 2nd rank stress tensor stored in row major format
         *
         * \f$f = \sigma^{ vonMises } + A*\sigma^{ mean } - B\f$
        ...
        tardigradeConstitutiveTools::computeUnitNormal(std::begin(jacobian), std::end(jacobian),
                                                       std::begin(unitDirection), std::end(unitDirection));
    ...
    void druckerPragerSurfaceFlatJ(const floatVector &stress, const floatType &A, const floatType &B,
                                   floatType &dpYield, floatVector &jacobian, floatVector &unitDirection,
                                   floatVector &unitDirectionJacobian) {
        /*!
         * Compute the Drucker-Prager yield criterion from a 2nd rank stress tensor stored in row major format
         *
         * \f$f = \sigma^{ vonMises } + A*\sigma^{ mean } - B\f$
    ...
    void druckerPragerSurface(const floatVector &stress, const floatType &A, const floatType &B, floatType &dpYield,
                              floatVector &jacobian, floatVector &unitDirection, floatMatrix &unitDirectionJacobian) {
        /*!
         * Compute the Drucker-Prager yield criterion from a 2nd rank stress tensor stored in row major format
         *
         * \f$f = \sigma^{ vonMises } + A*\sigma^{ mean } - B\f$
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(druckerPragerSurfaceFlatJ(stress, A, B, dpYield, jacobian, unitDirection,
                                                               _unitDirectionJacobian));

        TARDIGRADE_ERROR_TOOLS_CATCH(unitDirectionJacobian =
                                         tardigradeVectorTools::inflate(_unitDirectionJacobian, unitDirection.size(),
                                                                        stress.size()));
    ...
    void druckerPragerSurfaceFlatJ(const floatVector &stress, const floatVector &dpParam, floatType &dpYield,
                                   floatVector &jacobian, floatVector &unitDirection,
                                   floatVector &unitDirectionJacobian) {
        /*!
         * Compute the Drucker-Prager yield criterion from a 2nd rank stress tensor stored in row major format
         *
    ...
    void druckerPragerSurface(const floatVector &stress, const floatVector &dpParam, floatType &dpYield,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(druckerPragerSurfaceFlatJ(stress, dpParam, dpYield, jacobian, unitDirection,
                                                               _unitDirectionJacobian));
    ...
    void linearViscoelasticity(const floatType &currentTime, const floatVector &currentStrain,
                               ...
                               const floatType &alpha, floatVector &dStress, floatVector &stress,
                               floatVector &currentStateVariables) {
        /*!
         * Compute the stress for linear viscoelasticity based on the potential function
         *
         * \f$\rho^0 \Psi = 0.5*( E_{ IJ } G_{ \infty } E_{ IJ } + \sum_{ n=1 }^N ( E_{ IJ } - \Xi_{ IJ }^n ) G^n ( E_{
         * IJ } - \Xi_{ IJ } ) )\f$
         *
         ...
         * \param &dStress: The computed change in stress in the reference configuration ( i.e. the same configuration
         * as the strain ) \param &stress: The computed stress in the reference configuration ( i.e. the same
         * configuration as the strain ) \param &currentStateVariables: The current values of the state variables
         */

        floatType dt = currentTime - previousTime;

        // Check the material parameters
        TARDIGRADE_ERROR_TOOLS_CHECK(materialParameters.size() != 0,
                                     "At least one material parameter must be provided.");

        // Set the number of Prony-Series terms
        TARDIGRADE_ERROR_TOOLS_CHECK((materialParameters.size() - 1) % 2 == 0,
                                     "An equal number of taus and Gs must be provided.");
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(previousStateVariables.size() == nTerms * dim,
                                     "The number of previous state variables is not consistent with the strain size.");
    ...
    void linearViscoelasticity(const floatType &currentTime, const floatVector &currentStrain,
                               ...
                               const floatType &currentRateModifier, const floatType &previousRateModifier,
                               const floatVector &previousStateVariables, const floatVector &materialParameters,
                               const floatType &alpha, floatVector &stress, floatVector &currentStateVariables) {
        /*!
         * Compute the stress for linear viscoelasticity based on the potential function
         *
         * \f$\rho^0 \Psi = 0.5*( E_{ IJ } G_{ \infty } E_{ IJ } + \sum_{ n=1 }^N ( E_{ IJ } - \Xi_{ IJ }^n ) G^n ( E_{
         ...
         *         - \f$G\f$ s: The stiffness values
         * \param &alpha: The integration parameter ( 0 for implicit, 1 for explicit )
         * \param &stress: The computed stress in the reference configuration ( i.e. the same configuration as the
         * strain ) \param &currentStateVariables: The current values of the state variables
         */

        floatVector dStress;

        return linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain, currentRateModifier,
                                     previousRateModifier, previousStateVariables, materialParameters, alpha, dStress,
    ...
    void linearViscoelasticity(const floatType &currentTime, const floatVector &currentStrain,
                               ...
                               const floatVector &previousStateVariables, const floatVector &materialParameters,
                               const floatType &alpha, floatVector &dStress, floatVector &stress,
                               floatVector &currentStateVariables, floatMatrix &dstressdstrain,
         ...
         * \param &dStress: The computed change in stress in the reference configuration ( i.e. the same configuration
         * as the strain ) \param &stress: The computed stress in the reference configuration ( i.e. the same
         * configuration as the strain ) \param &currentStateVariables: The current values of the state variables \param
         * &dstressdstrain: The derivative of the stress w.r.t. the strain \param &dstressdrateModifier: The derivative
         * of the stress w.r.t. the rate modifier
         */
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain,
                                                           ...
                                                           previousStateVariables, materialParameters, alpha, dStress,
                                                           stress, currentStateVariables))
        ...
        for (unsigned int i = 1; i < nTerms + 1; i++) {
            ...
            dstressdrateModifier -=
                Gi * (dt * (1 - alpha) / (taui + dt * (1 - alpha) * currentRateModifier)) * (currentStrain - Xic);
        }
    ...
    void linearViscoelasticity(const floatType &currentTime, const floatVector &currentStrain,
         ...
         * \param &alpha: The integration parameter ( 0 for implicit, 1 for explicit )
         * \param &stress: The computed stress in the reference configuration ( i.e. the same configuration as the
         * strain ) \param &currentStateVariables: The current values of the state variables \param &dstressdstrain: The
         * derivative of the stress w.r.t. the strain \param &dstressdrateModifier: The derivative of the stress w.r.t.
         * the rate modifier
         */
        ...
        return linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain, currentRateModifier,
                                     previousRateModifier, previousStateVariables, materialParameters, alpha, dStress,
                                     stress, currentStateVariables, dstressdstrain, dstressdrateModifier);
    }
    ...
    void linearViscoelasticity(const floatType &currentTime, const floatVector &currentStrain,
                               ...
                               floatMatrix &dstressdPreviousStateVariables, floatMatrix &dStateVariablesdStrain,
                               floatVector &dStateVariablesdRateModifier, floatMatrix &dStateVariablesdPreviousStrain,
                               floatVector &dStateVariablesdPreviousRateModifier,
         ...
         * strain ) \param &currentStateVariables: The current values of the state variables \param &dstressdstrain: The
         * derivative of the stress w.r.t. the strain \param &dstressdrateModifier: The derivative of the stress w.r.t.
         * the rate modifier \param &dstressdPreviousStrain: The derivative of the stress w.r.t. the previous strain
         ...
         * \param &dStateVariablesdPreviousRateModifier: The derivative of the state variables w.r.t. the previous rate
         * modifier \param &dStateVariablesdPreviousStateVariables: The derivative of the state variables w.r.t. the
        ...
        return linearViscoelasticity(currentTime, currentStrain, previousTime, previousStrain, currentRateModifier,
                                     ...
                                     dstressdPreviousStateVariables, dStateVariablesdStrain,
                                     dStateVariablesdRateModifier, dStateVariablesdPreviousStrain,
                                     dStateVariablesdPreviousRateModifier, dStateVariablesdPreviousStateVariables);
    }
    ...
    void linearViscoelasticity(const floatType &currentTime, const floatVector &currentStrain,
                               ...
                               const floatType &alpha, floatVector &dStress, floatVector &stress,
                               floatVector &currentStateVariables, floatMatrix &dstressdstrain,
                               floatVector &dstressdrateModifier, floatMatrix &dstressdPreviousStrain,
                               floatVector &dstressdPreviousRateModifier, floatMatrix &dstressdPreviousStateVariables,
                               floatMatrix &dStateVariablesdStrain, floatVector &dStateVariablesdRateModifier,
                               floatMatrix &dStateVariablesdPreviousStrain,
                               floatVector &dStateVariablesdPreviousRateModifier,
                               floatMatrix &dStateVariablesdPreviousStateVariables) {
        /*!
         * Compute the stress for linear viscoelasticity based on the potential function
         ...
         *         - \f$G\f$ s: The stiffness values
         * \param &alpha: The integration parameter ( 0 for implicit, 1 for explicit )
         * \param &dStress: The change in stress in the reference configuration ( i.e., the same configuration as the
         * strain ) \param &stress: The computed stress in the reference configuration ( i.e., the same configuration as
         * the strain ) \param &currentStateVariables: The current values of the state variables \param &dstressdstrain:
         * The derivative of the stress w.r.t. the strain \param &dstressdrateModifier: The derivative of the stress
         * w.r.t. the rate modifier \param &dstressdPreviousStrain: The derivative of the stress w.r.t. the previous
         * strain \param &dstressdPreviousRateModifier: The derivative of the stress w.r.t. the previous rate modifier
         * \param &dstressdPreviousStateVariables: The derivative of the stress w.r.t. the previous state variables
         * \param &dStateVariablesdStrain: The derivative of the state variables w.r.t. the strain
         * \param &dStateVariablesdRateModifier: The derivative of the state variables w.r.t. the rate modifier
         * \param &dStateVariablesdPreviousStrain: The derivative of the state variables w.r.t. the previous strain
        ...
        dStateVariablesdPreviousStrain =
            floatMatrix(previousStateVariables.size(), floatVector(previousStrain.size(), 0));
        ...
        dStateVariablesdPreviousStateVariables =
            floatMatrix(previousStateVariables.size(), floatVector(previousStateVariables.size(), 0));
        ...
        for (unsigned int i = 1; i < nTerms + 1; i++) {
            ...
            dStateVariablesdCurrentRateModifier_matrix[i - 1] =
                dfactordr * ((1 - alpha) * (currentStrain - Xip) * currentRateModifier +
                             alpha * (previousStrain - Xip) * previousRateModifier) +
            ...
            floatType dxidPreviousXi_factor =
                -factor * ((1 - alpha) * currentRateModifier + alpha * previousRateModifier);
        ...
        dStateVariablesdPreviousRateModifier =
            tardigradeVectorTools::appendVectors(dStateVariablesdPreviousRateModifier_matrix);

        return;
    }

    void volumetricNeoHookean(const floatType &jacobian, const floatType &bulkModulus, floatType &meanStress) {
        /*!
    ...
    void perzynaModel(const floatType f, const floatType q, const floatType A, const floatVector &parameters,
                      floatType &p) {
        /*!
         * Implementation of the Perzyna type model of the form
         *
    ...
    void perzynaModel(const floatType f, const floatType q, const floatType A, const floatVector &parameters,
                      floatType &p, floatType &dpdf, floatType &dpdq, floatType &dpdA) {
        /*!
         * Implementation of the Perzyna type model of the form
         *
         * \f$p = A \left \langle \frac{ f }{ q } \right \rangle^n\f$
    ...
    void linearHardening(const floatVector &stateVariables, const floatVector &linearModuli,
                         const floatType &scalarShift, floatType &value) {
        /*!
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(stateVariables.size() == linearModuli.size(),
                                     "The state variables and the moduli must have the same size");
    ...
    void linearHardening(const floatVector &stateVariables, const floatVector &linearModuli,
                         const floatType &scalarShift, floatType &value, floatVector &valueJacobian) {
        /*!
         * Compute the linear hardening curve value.
         *
         * \f$value = stateVariables_i linearModuli_i + scalarShift\f$
    ...
    void computeJaumannStiffnessTensor(const floatVector &cauchyStress, const floatVector &currentDeformationGradient,
         ...
         * \f$J \mathbb{ C }_{ ijkl } \delta D_{ kl } = \delta \left( J \sigma_{ ij }\right) + \delta W_{ ik } \sigma_{
         * kj } - \sigma_{ ik } \delta W_{ kj }\f$
         *
         ...
         *
         * \f$\mathbb{ C }_{ ijkl } = \left( \delta_{ mn } \sigma_{ ij } + \frac{ D \sigma_{ ij } }{ D F_{ mK } } F_{ nK
         * }\right)\mathbb{ P }_{ mnkl }^{ symm }\f$
         *
         * Where
         *
         * \f$\mathbb{ P }_{ ijkl }^{ symm } = \frac{ 1 }{ 2 }\left( \delta_{ ik } \delta_{ jl } + \delta_{ jk }
         * \delta_{ il }\right)\f$
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(cauchyStress.size() == currentDeformationGradient.size(),
                                     "The cauchy stress ( length " + std::to_string(cauchyStress.size()) +
                                         " ) and the deformation gradient ( length " +
                                         std::to_string(currentDeformationGradient.size()) +
                                         " ) must have the same size")

        TARDIGRADE_ERROR_TOOLS_CHECK(cauchyStress.size() == dCauchydF.size(),
                                     "The derivative of the Cauchy stress w.r.t. the deformation gradient has " +
        ...
        for (unsigned int i = 0; i < dCauchydF.size(); i++) {
            TARDIGRADE_ERROR_TOOLS_CHECK(dCauchydF[i].size() == currentDeformationGradient.size(),
                                         "Row " + std::to_string(i) + " of dCauchydF is of length " +
                                             std::to_string(dCauchydF[i].size()) + " and it should have a length of " +
                                             std::to_string(currentDeformationGradient.size()))
        }
